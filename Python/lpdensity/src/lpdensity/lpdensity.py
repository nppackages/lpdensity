...
def lpdensity(
    ...
    if not np.isreal(nUniqueMin) or np.isnan(nUniqueMin):
        ...
        if abs(sum(Cweights * Pweights)) <= np.finfo(float).eps * 10:
            raise Exception(
                "Composited weights (Cweights * Pweights) are numerically zero."
            )
    ...
    if bwselect == "mse-dpi":
        bw = lpbwdensity_fn.__bw_MSE(
            ...
            nLocalMin=nLocalMin,
            nUniqueMin=nUniqueMin,
        )
    elif bwselect == "imse-dpi":
        bw = lpbwdensity_fn.__bw_IMSE(
            data=data,
            grid=grid,
            p=p,
            v=v,
    ...
    ##   Point Estimation   ##
    ##########################
    Temp_Result = lpdensity_fn.__lpdensity_fn(
        data=data,
        grid=grid,
        bw=bw,
        p=p,
    ...
    # scaling output
    Temp_Result["Estimate"][["f_p", "f_q", "se_p", "se_q"]] = (
        Temp_Result["Estimate"][["f_p", "f_q", "se_p", "se_q"]] * scale
    )
    ...
    lpoutput = lpdensity_output(
        Temp_Result["Estimate"],
        Temp_Result["CovMat_p"],
        Temp_Result["CovMat_q"],
        p,
        q,
        v,
        kernel,
...
class lpdensity_output:
    ...
    def __repr__(self):
        ...
        print(
            "Polynomial order for point estimation    (p=):".ljust(fw),
            str(self.p).rjust(9),
        )
        print(
            "Order of derivative estimated            (v=):".ljust(fw),
            str(self.v).rjust(9),
        )
        print(
            "Polynomial order for confidence interval (q=):".ljust(fw),
            str(self.q).rjust(9),
        )
        ...
        if CIuniform == True:
            if np.isnan(CIsimul):
                raise warnings.warn(
                    "Option CIsimul incorrectly specified. Will only plot pointwise confidence intervals."
                )
                ...
                corrMat = self.CovMat_q.mul(1 / self.Estimate["se_q"], axis=0).mul(
                    1 / self.Estimate["se_q"], axis=1
                )
                # cehck PSD matrix
                if np.all(np.linalg.eigvals(corrMat) >= 0):
                    # simulate multivariate normal
                    normalSimul = np.random.multivariate_normal(
                        mean=np.repeat(0, len(self.CovMat_q)), cov=corrMat, size=2000
                    )
                    # comput estimated quantile
                    z_val = np.quantile(
                        list(map(lambda x: max(abs(x)), normalSimul)), 1 - alpha
                    )

                else:
                    # exception for non-PSD covariance matrix
                    raise warnings.warn(
                        "Variance-Covariance is not positive semidefinite. Will only plot pointwise confidence intervals."
                    )
                    z_val = spstat.norm.ppf(1 - alpha / 2)
        else:
            z_val = spstat.norm.ppf(1 - alpha / 2)
        ...
        if self.p == self.q:
            ci_df["CI_l_q"] = self.Estimate["f_p"] - z_val * self.Estimate["se_p"]
            ci_df["CI_r_q"] = self.Estimate["f_p"] + z_val * self.Estimate["se_p"]
        else:
        ...
        for j in range(self.ng):
            print(
                str(j + 1).ljust(fw_c),
                str(round(self.Estimate["grid"][j], n_dec)).ljust(fw_c),
                str(round(self.Estimate["bw"][j], n_dec)).ljust(fw_c),
                str(int(self.Estimate["nh"][j])).ljust(fw_c),
                str(int(self.Estimate["nhu"][j])).ljust(fw_c),
                str(round(self.Estimate["f_p"][j], n_dec)).rjust(fw_c),
                str(round(self.Estimate["se_p"][j], n_dec)).rjust(fw_c),
                str(
                    "["
                    + str(round(ci_df["CI_l_q"][j], n_dec))
                    + ","
                    + str(round(ci_df["CI_r_q"][j], n_dec))
                    + "]"
                ).rjust(fw_l),
            )
        print("=" * 81)
    ...
    def vcov(self):
        ...
        return {
            "stdErr": vcov_data,
            "CovMat_p": self.CovMat_p,
            "CovMat_q": self.CovMat_q,
        }
    ...
    def confint(self, alpha=0.05, CIuniform=False, CIsimul=2000):
        ...
        estimate_df = pd.DataFrame(
            columns=["grid", "f_p", "CI_l_p", "CI_r_p", "f_q", "CI_l_q", "CI_r_q"]
        )
        ...
        if CIuniform == True:
            if np.isnan(CIsimul):
                ...
                if np.all(np.linalg.eigvals(corrMat) >= 0):
                    # simulate multivariate normal
                    normalSimul = np.random.multivariate_normal(
                        mean=np.repeat(0, len(self.CovMat_q)), cov=corrMat, size=CIsimul
                    )
                    # comput estimated quantile
                    z_val = np.quantile(
                        list(map(lambda x: max(abs(x)), normalSimul)), 1 - alpha
    ...
    def plot(
        ...
        if hist == True and histData is not None:
            ...
            if histBins is None:
                histBreaks = 25
                temp_plot = (
                    pn.ggplot(histData, pn.aes(x="d1"))
                    + pn.geom_histogram(bins=histBins)
                    + pn.theme_bw()
                )
        else:
        ...
        if CItype in ["region", "all"]:
            temp_plot = temp_plot + pn.geom_ribbon(
                data=data_x,
                mapping=pn.aes(x="grid", ymin="CI_l", ymax="CI_r"),
                alpha=CIshade,
            )
        ...
        if CItype in ["lines", "all"]:
            temp_plot = (
                temp_plot
                + pn.geom_line(data=data_x, mapping=pn.aes(x="grid", y="CI_l"))
                + pn.geom_line(data=data_x, mapping=pn.aes(x="grid", y="CI_r"))
            )

        if CItype in ["ebar", "all"]:
            temp_plot = temp_plot + pn.geom_errorbar(
                data=data_x, mapping=pn.aes(x="grid", ymin="CI_l", ymax="CI_r")
            )

        if type in ["line", "both"]:
            temp_plot = temp_plot + pn.geom_line(
                data=data_x, mapping=pn.aes(x="grid", y="f_p")
            )

        if type in ["points", "both"]:
            temp_plot = temp_plot + pn.geom_point(
                data=data_x, mapping=pn.aes(x="grid", y="f_p")
            )
