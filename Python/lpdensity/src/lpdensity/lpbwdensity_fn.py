...
def __lpdensityUnique(x):
    ...
    unique, uniqueIndex, nUniquelist = np.unique(
        x, return_index=True, return_counts=True, axis=0
    )
    nUnique = sum(unique)
...
def __normal_dgps(x_val, v, mean, sd):
    if v == 0:
        ...
        temp = sympy.exp(-((x - mean) ** 2) / (2 * sd**2)) / sympy.sqrt(
            2 * math.pi * sd**2
        )
        while v > 1:
    ...
    for i in range(1, p + 2):
        for j in range(1, p + 2):
            if kernel == "uniform":
                ...
                integrand = lambda l: np.power(0.75 * (1 - l**2), 2) * np.power(
                    l, i + j - 2
                )
            else:
...
def __Ggenerate(p, low=-1, up=1, kernel="uniform"):
    ...
    for i in range(1, p + 2):
        for j in range(1, p + 2):
            ...
            def integrand_1(x, y):
                return (
                    np.power(x, i) * np.power(y, (j - 1)) * (1 - abs(x)) * (1 - abs(y))
                )
            ...
            G[i - 1, j - 1] = (
                integrate.quad(x_integral_1, low, up)[0]
                + integrate.quad(x_integral_2, low, up)[0]
            )
...
def __bw_ROT(
    ...
    sd_hat = np.sqrt(
        sum(Cweights * Pweights * (data - mean_hat) ** 2) / sum(Cweights * Pweights)
    )

    # normal quantities
    temp_1 = sympy.exp(-((x - mean_hat) ** 2) / (2 * sd_hat**2)) / sympy.sqrt(
        2 * math.pi * sd_hat**2
    )
    temp_2 = sympy.diff(temp_1, x)
    temp_3 = sympy.exp(-((x - mean_hat) ** 2) / (2 * sd_hat**2)) / sympy.sqrt(
        2 * math.pi * sd_hat**2
    )
    temp_4 = sympy.exp(-((x - mean_hat) ** 2) / (2 * sd_hat**2)) / sympy.sqrt(
        2 * math.pi * sd_hat**2
    )
    ...
    for j in range(ng):
        bias_dgp[j, 0] = (
            temp_3.evalf(subs={x: grid[j]}) / math.factorial(p + 1) * math.factorial(v)
        )
        bias_dgp[j, 1] = temp_4.evalf(subs={x: grid[j]}) / math.factorial(
            p + 2
        ) * math.factorial(v) + bias_dgp[j, 0] * temp_2.evalf(
            subs={x: grid[j]}
        ) / temp_1.evalf(subs={x: grid[j]})
    ...
    if v > 0:
        ...
        for j in range(ng):
            ...
            sd_dgp[j, 0] = np.sqrt(
                norm.cdf(grid[j], loc=mean_hat, scale=sd_hat)
                * (1 - norm.cdf(grid[j], loc=mean_hat, scale=sd_hat))
                / norm.pdf(grid[j], loc=mean_hat, scale=sd_hat)
                / (0.5 * (n**2))
            )
    ...
    for j in range(ng):
        if v > 0:
            opt_f = lambda a: np.power(a, 2 * p + 2 - 2 * v) * (
                bias_dgp[j, 0] + a * bias_dgp[j, 1]
            ) ** 2 + sd_dgp[j, 0] ** 2 / np.power(a, 2 * v - 1)
        else:
            opt_f = (
                lambda a: np.power(a, 2 * p + 2 - 2 * v)
                * (bias_dgp[j, 0] + a * bias_dgp[j, 1]) ** 2
                + sd_dgp[j, 0] ** 2 / a
            )
        h[j] = optimize.minimize_scalar(
            opt_f, bounds=(0, max(data) - min(data)), method="bounded"
        ).x
    ...
    for j in range(ng):
        ...
        if regularize == True:
            ...
            if nUniqueMin > 0:
                h[j] = max(
                    h[j],
                    np.sort(abs(dataUnique - grid[j]))[min(nUnique, nUniqueMin) - 1],
                )
...
def __bw_IROT(
    ...
    # bandwidth
    if v > 0:
        opt_f = lambda a: np.power(a, 2 * p + 2 - 2 * v) * sum(
            (bias_dgp[:, 0] + a * bias_dgp[:, 1]) ** 2
        ) + sum(sd_dgp[:, 0] ** 2) / np.power(a, 2 * v - 1)
    else:
        opt_f = (
            lambda a: np.power(a, 2 * p + 2 - 2 * v)
            * sum((bias_dgp[:, 0] + a * bias_dgp[:, 1]) ** 2)
            + sum(sd_dgp[:, 0] ** 2) / a
        )
    ...
    if regularize == True:
        for j in range(ng):
            ...
            if nUniqueMin > 0:
                h = max(
                    h, np.sort(abs(dataUnique - grid[j]))[min(nUnique, nUniqueMin) - 1]
                )
...
def __bw_MSE(
    ...
    if massPoints == True:
        Fn = np.repeat(
            (np.cumsum(Cweights * Pweights) / sum(Cweights * Pweights))[indexUnique],
            repeats=freqUnique,
        )
    else:
    ...
    if nUnique > 1:
        weights_normalUnique = weights_normalUnique - np.insert(
            weights_normalUnique[0 : (nUnique - 1)], 0, 0
        )

    CweightsUnique = np.cumsum(Cweights)[indexUnique]
    if nUnique > 1:
        CweightsUnique = CweightsUnique - np.insert(
            CweightsUnique[0 : (nUnique - 1)], 0, 0
        )

    PweightsUnique = np.cumsum(Pweights)[indexUnique]
    if nUnique > 1:
        PweightsUnique = PweightsUnique - np.insert(
            PweightsUnique[0 : (nUnique - 1)], 0, 0
        )
    ...
    for j in range(ng):
            ...
            temp = (
                np.matmul(
                    np.matmul(
                        inv(
                            np.matmul(
                                Xh_p_temp.transpose(), Xh_p_temp.mul(Kh_temp, axis=0)
                            )
                        ),
                        Xh_p_temp.transpose(),
                    ),
                    Y_temp.mul(Kh_temp, axis=0),
                )
            ).loc[p + 2, 0] / np.power(hp2, p + 2)
        ...
        # Cp matrix
        if p > 0:
            C_p_hat = pd.DataFrame(
                Xh_temp[np.repeat(Xh_temp.columns, p + 1)]
                .apply(lambda x: np.power(x, range(p + 1, 2 * p + 1 + 1)), axis=1)
                .mul(Kh_temp.mul(Pweights[index_temp], axis=0), axis=0)
                .sum(axis=0)
                / n
            )
        else:
            C_p_hat = pd.DataFrame(
                Xh_temp.apply(lambda x: np.power(x, 1), axis=1)
                .mul(Kh_temp.mul(Pweights[index_temp], axis=0), axis=0)
                .sum(axis=0)
                / n
            )
        ...
        # Cp+1 matrix
        if p > 0:
            C_p1_hat = pd.DataFrame(
                Xh_temp[np.repeat(Xh_temp.columns, p + 1)]
                .apply(lambda x: np.power(x, range(p + 2, 2 * p + 2 + 1)), axis=1)
                .mul(Kh_temp.mul(Pweights[index_temp], axis=0), axis=0)
                .sum(axis=0)
                / n
            )
            ...
            C_p1_hat = pd.DataFrame(
                Xh_temp.apply(lambda x: np.power(x, 2), axis=1)
                .mul(Kh_temp.mul(Pweights[index_temp], axis=0), axis=0)
                .sum(axis=0)
                / n
            )
        ...
        # S matrix
        Xh_p_temp = pd.DataFrame(
            Xh_temp[np.repeat(Xh_temp.columns, p + 1)].apply(
                lambda x: np.power(x, range(p + 1)), axis=1
            )
        )
        ...
        S_hat = (
            np.matmul(
                Xh_p_temp.transpose(),
                Xh_p_temp.mul(Kh_temp.mul(Pweights[index_temp], axis=0), axis=0),
            )
            / n
        )
        ...
        if v == 0:
            ...
            if massPoints == True:
                ...
                F_Xh_p_Kh_temp = (
                    np.matmul(Y_temp.transpose(), Xh_p_Kh_Pweights_temp) / n
                )
                ...
                for jj in range(len(Xh_p_Kh_Pweights_temp.columns)):
                    G[:, jj] = (
                        np.repeat(
                            (np.cumsum(Xh_p_Kh_Pweights_temp.iloc[::-1, jj]) / n)[::-1],
                            freqUnique,
                        )
                        - F_Xh_p_Kh_temp.iloc[0, jj]
                    ) * weights_normal
                ...
                for jj in range(len(Xh_p_Kh_Pweights_temp.columns)):
                    G[:, jj] = (
                        np.repeat(
                            np.cumsum(Xh_p_Kh_Pweights_temp.iloc[:, jj] / n), freqUnique
                        )
                        - F_Xh_p_Kh_temp.iloc[0, jj]
                    ) * weights_normal
        ...
        if v > 0:
            const_hat[j, 2] = math.factorial(v) * np.sqrt(
                abs(np.matmul(S_hat_inv, np.matmul(G_hat, S_hat_inv)).loc[v, v])
                / (n * h1)
            )
        else:
            ...
            const_hat[j, 2] = math.factorial(v) * np.sqrt(
                abs(
                    np.matmul(S_hat_inv, np.matmul(G_hat, S_hat_inv)).loc[v, v]
                    / (0.5 * n**2)
                    * h1
                    * temp_ii
                    * (1 - temp_ii)
        ...
        if v > 0:
            opt_f = lambda a: np.power(a, 2 * p + 2 - 2 * v) * (
                dgp_hat[j, 0] * const_hat[j, 0] + a * dgp_hat[j, 1] * const_hat[j, 1]
            ) ** 2 + const_hat[j, 2] ** 2 / np.power(a, 2 * v - 1)
        else:
            opt_f = (
                lambda a: np.power(a, 2 * p + 2 - 2 * v)
                * (
                    dgp_hat[j, 0] * const_hat[j, 0]
                    + a * dgp_hat[j, 1] * const_hat[j, 1]
                )
                ** 2
                + const_hat[j, 2] ** 2 / a
            )
    ...
    for j in range(ng):
        ...
        if regularize == True:
            if nLocalMin > 0:
                h[j] = max(
                    h[j], np.sort(abs(data - grid[j]))[min(n - 1, nLocalMin - 1)]
                )
            if nUniqueMin > 0:
                h[j] = max(
                    h[j],
                    sorted(abs(dataUnique - grid[j]))[min(nUnique - 1, nUniqueMin - 1)],
                )
...
def __bw_IMSE(
        ...
        if v == 0:
            ...
            if massPoints == True:
                ...
                for jj in range(len(Xh_p_Kh_Pweights_temp.columns)):
                    G[:, jj] = (
                        np.repeat(
                            (np.cumsum(Xh_p_Kh_Pweights_temp.iloc[::-1, jj - 2]) / n)[
                                ::-1
                            ],
                            freqUnique,
                        )
                ...
                for jj in range(len(Xh_p_Kh_Pweights_temp.columns)):
                    G[:, jj] = (
                        np.cumsum(Xh_p_Kh_Pweights_temp.iloc[:, jj] / n)
                        - F_Xh_p_Kh_temp.iloc[0, jj]
                    ) * weights_normal
    ...
    if v > 0:
        opt_f = lambda a: np.power(a, 2 * p + 2 - 2 * v) * sum(
            (dgp_hat[:, 0] * const_hat[:, 0] + a * dgp_hat[:, 1] * const_hat[:, 1]) ** 2
        ) + sum(const_hat[:, 2] ** 2) / np.power(a, 2 * v - 1)
    else:
        opt_f = (
            lambda a: np.power(a, 2 * p + 2 - 2 * v)
            * sum(
                (dgp_hat[:, 0] * const_hat[:, 0] + a * dgp_hat[:, 1] * const_hat[:, 1])
                ** 2
            )
            + sum(const_hat[:, 2] ** 2) / a
        )

    h = optimize.minimize_scalar(
        opt_f, bounds=(0, max(data) - min(data)), method="bounded"
    ).x
