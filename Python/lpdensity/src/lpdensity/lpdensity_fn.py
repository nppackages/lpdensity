...
def __lpdensity_fn(
    data, grid, bw, p, q, v, kernel, Cweights, Pweights, massPoints, showSE=True
):
    # data prep
    ii = np.argsort(data)
    data = data[ii]
    ...
    for j in range(ng):
        ...
        if massPoints == True:
            ...
            try:
                XhKhXh_inv = inv(
                    np.matmul(
                        Xh_p_temp[index_temp[indexUnique]].transpose(),
                        Xh_p_Kh_Pweights_temp[index_temp[indexUnique]],
                    )
                    / n
                )
            ...
            hat_p[j] = (
                math.factorial(v)
                * np.matmul(
                    Y_temp.transpose(), np.matmul(Xh_p_Kh_Pweights_temp, XhKhXh_inv)
                )[v]
                / (np.power(bw[j], v))
                / n
            )
            ...
            try:
                XhKhXh_inv = inv(
                    np.matmul(
                        Xh_p_temp[index_temp].transpose(),
                        Xh_p_Kh_Pweights_temp[index_temp],
                    )
                    / n
                )
            except np.linalg.LinAlgError:
            ...
            hat_p[j] = (
                ...
                * np.matmul(
                    XhKhXh_inv,
                    np.matmul(
                        Xh_p_Kh_Pweights_temp[index_temp, :].transpose(),
                        Y_temp[index_temp],
                    ),
                )[v]
                / (np.power(bw[j], v))
        ...
        if showSE == True:
            if massPoints == True:
                ...
                iff_p[:, j] = (
                    np.matmul(XhKhXh_inv, G.transpose()).iloc[v, :]
                    * math.factorial(v)
                    / np.sqrt(n * np.power(bw[j], 2 * v))
                )
            else:
                F_Xh_p_Kh_temp = (
                    np.matmul(Y_temp.transpose(), Xh_p_Kh_Pweights_temp) / n
        ...
        if q > p:
            if massPoints == True:
                ...
                try:
                    XhKhXh_inv = inv(
                        np.matmul(
                            Xh_q_temp[index_temp[indexUnique]].transpose(),
                            Xh_q_Kh_Pweights_temp[index_temp[indexUnique]],
                        )
                        / n
                    )
                except np.linalg.LinAlgError:
                ...
                hat_q[j] = (
                    math.factorial(v)
                    * np.matmul(
                        Y_temp.transpose(), np.matmul(Xh_q_Kh_Pweights_temp, XhKhXh_inv)
                    )[v]
                    / (np.power(bw[j], v))
                    / n
                )
                ...
                try:
                    XhKhXh_inv = inv(
                        np.matmul(
                            Xh_q_temp[index_temp].transpose(),
                            Xh_q_Kh_Pweights_temp[index_temp],
                        )
                        / n
                    )
                except np.linalg.LinAlgError:
                ...
                hat_q[j] = (
                    ...
                    * np.matmul(
                        XhKhXh_inv,
                        np.matmul(
                            Xh_q_Kh_Pweights_temp[index_temp, :].transpose(),
                            Y_temp[index_temp],
                        ),
                    )[v]
                    / (np.power(bw[j], v))
            ...
            if showSE == True:
                if massPoints == True:
                    F_Xh_q_Kh_temp = (
                        np.matmul(Y_temp.transpose(), Xh_q_Kh_Pweights_temp) / n
                    )
                    ...
                    for jj in range(len(Xh_q_Kh_Pweights_temp.columns)):
                        G[:, jj] = (
                            np.repeat(
                                (np.cumsum(Xh_q_Kh_Pweights_temp.iloc[::-1, jj]) / n)[
                                    ::-1
                                ],
                                freqUnique,
                            )
                            - F_Xh_q_Kh_temp.iloc[0, jj]
                        ) * weights_normal
                    ...
                    for jj in range(len(Xh_q_Kh_Pweights_temp.columns)):
                        G[:, jj] = (
                            np.repeat(
                                np.cumsum(Xh_q_Kh_Pweights_temp.iloc[:, jj] / n),
                                freqUnique,
                            )
                            - F_Xh_q_Kh_temp.iloc[0, jj]
                        ) * weights_normal
    ...
    est_data = pd.DataFrame(
        data=Estimate, columns=["grid", "bw", "nh", "nhu", "f_p", "f_q", "se_p", "se_q"]
    )
